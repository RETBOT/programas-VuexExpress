  ...
  passport.use(new EstrategiaLocal({
    ...
    Usuario.obtenerUsuarioporEmail(email, (err, usuario) => {
      if (err) { return hecho(err); }
        ...
        Usuario.compararContrasenhas(contrasenha, usuario.contrasenha, (error, coincide) => {
          if (coincide) {
  ...
  app.post('/usuarios/registro', (req, res) => {
    ...
    Usuario.crearUsuario(nuevoUsuario, (error, usuario) => {
      if (error) { 
        console.log(error); 
        res.status(422).json({
          mensaje: 'Algo salió mal, por favor intenetelo nuevamente después de unos minutos',
        });
      }
      else {
        res.send({ usuario });
